...
4. Map independent decision-changing uncertainties. When subagent workers are requested, parallelize useful independent questions where that improves the investigation and queue remaining questions as needed; available capacity is not a quota to fill. Leave worker model/class selection to the agent and harness defaults; honor explicit operator overrides. Each brief names the question, source domain, governing decisions, exclusive artifact path, permitted writer/checkpoint channel and exit evidence. Parallelize coverage, not repeated votes on the same question.
