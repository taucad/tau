...
- `extra_body.google.stream_function_call_arguments: true` rides every streamed Vertex request and splits each tool call's arguments into four deltas instead of one; it is valid only on a streamed request, and a unary one answers 400. Vertex sheds requests carrying it with 499 CANCELLED while its shared-quota pool is stressed — near-deterministic inside such a window, 0/120 outside one on Tau's production body — so `executeGatewayProviderRequest` re-dispatches a 499 once without it. One retry, no circuit breaker; do not add a second.
