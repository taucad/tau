...
The collector settles fonts, images, animations, lazy scrolling, and routed requests; then sequentially visits native `<details>`, ARIA accordions with `aria-expanded` plus `aria-controls`, and ARIA tabs with `role=tab` plus `aria-controls`. It captures the baseline first, restores prior state where possible, deduplicates captured panels, and reports discovered, visited, empty, failed, and skipped states. Failed or malformed recognized states preserve the baseline and produce an explicit `partial` report; arbitrary widgets are never clicked.

Only credential-free public HTTP(S) resources are allowed. The `html-v3` profile counts every routed request before classification, charges all received bytes, and records fixed omission counters. Generated Markdown exposes `requests`, `media`, `peripheral`, `capabilities`, `subresources`, `subframes`, `non-reading`, and `images` on one `Evidence loss` line. Media, text tracks, and manifests are aborted before network fetch. Failed optional subresources, subframes, non-reading requests, broken images, and safely blocked ambient workers, sockets, transports, beacons, and service workers preserve usable evidence as `partial`; surrounding captions, alt text, and prose remain. Main-document trust failures, global budgets, unknown errors, popups, downloads, dialogs, file pickers, invalid artifacts, and rights failures remain fatal. Authentication, cookies, consent clicks, arbitrary selectors, media payload capture, active/raw HTML, HAR/WARC/MHTML, and replay are unsupported. The PDF preserves visual evidence, including rendered SVG/canvas/WebGL output where the page exposes it to Chromium; it never contains live JavaScript.
