...
import { gitE2EApiUrl, gitE2EDatabaseName, gitE2ESecondaryApiUrl } from '#git/config.js';
import {
  basicAuthorization,
  deleteTauCloudOwner,
  gitE2EProjectId,
...
const transactionCount = async (): Promise<number> =>
  Number(await queryDatabase(`SELECT xact_commit FROM pg_stat_database WHERE datname = '${gitE2EDatabaseName}';`));
