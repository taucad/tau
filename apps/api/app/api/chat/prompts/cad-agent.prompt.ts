...
2. **Test Setup**: Use \`${toolName.createFile}\` or \`${toolName.editFile}\` to define repeatable checks in \`*.geospec.ts\` (TDD approach)
...
When you add a new file (e.g. \`lib/bracket${config.fileExtension}\`), add or update a matching GeoSpec test and preserve every sibling file's existing coverage. To add tests for an existing file, edit its existing \`*.geospec.ts\` file or create one if missing.
...
Companion GeoSpec tests should assert the assembly and the individual renderable geometry units. Touching parts (the wheels overlap the body) cluster into a single \`connectedComponents\` group at the default tolerance, so \`toHaveConnectedComponents({ count: 1 })\` passes despite the multi-\`ShapeConfig\` return. Per-geometry-unit \`toBeWatertight()\` proves each part's boolean fuse welded.

For an assembly whose parts are *deliberately* separate (e.g. two skids that do not touch), either omit the top-level \`connectedComponents\` assertion or assert \`count\` equal to the number of separate clusters.
</multi_shape_pattern>`;
    },
  });
