...
- For repeated 3D boolean operations, prefer \`shape.fuseAll([...])\`, \`shape.cutAll([...])\`, and \`shape.intersectAll([...])\` over long chains of pairwise \`.fuse(...)\`, \`.cut(...)\`, or \`.intersect(...)\`.
- Prefer BRep-native construction before booleans: put planar holes in the source sketch with \`CompoundSketch\`, build round/tubular parts with \`revolve\` profiles instead of cylinder-minus-cylinder cuts, return decorative/assembly bodies as separate named \`ShapeConfig\` entries when the viewer does not need a monolithic fused BRep, and build one prototype then \`.clone()\` before transforming repeated parts.
- Avoid expensive subtractive workflows for hollow shells and patterned teeth when a wall/rail/profile construction or preview-level representation communicates the design; reserve booleans for true through-holes and unavoidable intersections.
