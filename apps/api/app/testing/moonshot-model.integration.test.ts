...
const modelId = 'moonshot-kimi-k3';
...
describe.skipIf(requiresEnv('MOONSHOT_API_KEY'))('Moonshot Kimi K3 live integration', () => {
  let testApp: TestApp;
  ...
  it('should stream reasoning and final text through Tau UI messages', async () => {
    const response = await fetch(`${testApp.baseUrl}/v1/chat`, {
      ...
      body: JSON.stringify({
        id: `moonshot-reasoning-${Date.now()}`,
        messages: [
          {
            id: 'msg_1',
            role: 'user',
  ...
  it('should complete a two-call Tau tool loop with reasoning replay', async () => {
    const response = await fetch(`${testApp.baseUrl}/v1/chat`, {
      ...
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        id: `moonshot-tool-loop-${Date.now()}`,
        messages: [
          {
            id: 'msg_1',
            role: 'user',
  ...
  it('should expose repeated-prefix cache reads through Tau billing usage', async () => {
    const threadId = `moonshot-cache-${Date.now()}`;
    ...
    expect(
      ...
      'Expected Moonshot cache reads in Tau usage data',
    ).toBe(true);
  }, 180_000);
