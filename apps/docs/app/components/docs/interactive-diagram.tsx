    ...
    <>
      ...
      {data.description && (
        <NodeToolbar isVisible={isHovered} position={Position.Bottom} offset={8}>
          <div className='z-50 w-64 rounded-lg border border-border bg-popover p-3 text-xs leading-relaxed text-popover-foreground shadow-menu'>
            {data.description}
          </div>
        </NodeToolbar>
      )}
...
function InteractiveDiagramRenderer({
  ...
  const colorMode: ColorMode =
    resolvedTheme === 'dark' || resolvedTheme === 'black' || resolvedTheme === 'high-contrast' ? 'dark' : 'light';
