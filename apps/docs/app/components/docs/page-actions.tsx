...
export function DocsPageActions(): React.JSX.Element {
  ...
  return (
    <div className='mt-5 space-y-1 border-t border-border pt-3 text-sm'>
      <button
        type='button'
        className='flex min-h-9 w-full items-center rounded-md px-2 text-left text-muted-foreground hover:bg-accent hover:text-foreground focus-visible:focus-outline'
        onClick={() => {
          void copyMarkdown();
        }}
      >
      ...
      <a
        className='flex min-h-9 items-center rounded-md px-2 text-muted-foreground hover:bg-accent hover:text-foreground focus-visible:focus-outline'
        href={`${url}.mdx`}
      >
        View as markdown
      </a>
