...
Both are offered wherever the host advertises them; Tau Desktop and `tau serve` advertise both. `.tau/chats`, `.tau/workspaces` and `.tau/revisions` are readable to the agent and refuse its writes.
