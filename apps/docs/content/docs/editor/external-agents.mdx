...
description: Run Codex inside a Tau project through its own CLI, its own login, and its own bill.
docType: reference
...
An older CLI is refused before the turn starts, naming the version it found. Codex is the qualified path; Claude Code is discovered and offered, and its live turns are not yet qualified.
...
Each agent keeps its own configuration directory — `CODEX_HOME`, `CLAUDE_CONFIG_DIR`. Nothing else about your environment travels: no Tau credential and no provider key reaches the child process.
