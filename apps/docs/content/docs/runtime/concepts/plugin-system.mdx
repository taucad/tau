...
Native-language kernels keep the same public lifecycle: [`@taucad/build123d`](../reference/build123d) starts its private Python child from `initialize()` and terminates the descendant tree from `cleanup()`. This is not a public runner plugin role -- the language protocol, process supervision, and artifact channel stay package-owned.
