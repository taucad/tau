...
function generateLibraryTracingSection(results: BenchmarkResult[]): string {
  const withLibrary = results.filter((r) => r.librarySummary);
  if (withLibrary.length === 0) {
    return '';
  }

  let sections = '';
  for (const result of withLibrary) {
    if (!result.librarySummary) {
      continue;
    }
  ...
  return `<h2>Replicad Library Tracing</h2>${sections}`;
}
...
<body>
  ...
  ${generateLibraryTracingSection(run.results)}
