...
const replicadRuntime = defineRuntime({ kernels: [replicadKernel()], bundlers: [esbuildBundler()] });
const opencascadeRuntime = defineRuntime({ kernels: [opencascadeKernel()], bundlers: [esbuildBundler()] });
...
describe('Cross-kernel mesh parity', { timeout: 120_000 }, () => {
  it('should produce matching normals and positions for a cylinder', async () => {
    const replicadResult = await createTestGeometry({
      runtime: replicadRuntime,
      files: {
        'cylinder.ts': `
...
export default function main() {
  return makeCylinder(5, 20);
    ...
    const occtResult = await createTestGeometry({
      runtime: opencascadeRuntime,
      files: {
        'cylinder.ts': `
