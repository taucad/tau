...
test('development StrictMode keeps the root headless service usable', async () => {
  ...
  expect(await target.evaluate(() => 'gpu' in navigator)).toBe(true);
  ...
  expect(evidence.mimeType).toBe('image/webp');
  expect(Math.max(evidence.width, evidence.height)).toBe(2400);
