...
export type SectionColorEvidence = Readonly<{
  darkRed: number;
  yellow: number;
  cyanOrBlue: number;
  black: number;
...
export const readSectionColorEvidence = async (image: Locator): Promise<SectionColorEvidence> =>
  target.evaluateLocator(image, async (element) => {
    const imageElement = element as HTMLImageElement;
    const response = await fetch(imageElement.currentSrc || imageElement.src);
    ...
    if (!context) {
      bitmap.close();
      throw new Error('Section evidence canvas is unavailable');
    }
    context.drawImage(bitmap, 0, 0);
    bitmap.close();
    const pixels = context.getImageData(0, 0, canvas.width, canvas.height).data;
    const counts = { black: 0, cyanOrBlue: 0, darkRed: 0, yellow: 0 };
    for (let y = 0; y < canvas.height; y += 2) {
      for (let x = 0; x < canvas.width; x += 2) {
        const normalizedX = x / canvas.width;
        const normalizedY = y / canvas.height;
        ...
        if (blue > 80 && blue > red * 1.25 && green > red * 1.1) {
          counts.cyanOrBlue++;
        }
...
export const waitForCaptureAttachments = async (count: number, timeout = 120_000): Promise<void> => {
  try {
    await target.evaluate(
