...
/**
 * Chats with a dispatch between `prepare` and the wire call, keyed by chat.
 *
 * ponytail: a module-level set, because "one dispatch at a time" is a fact
 * about the *chat*, not about a component — it used to be a `useRef` in a hook
 * mounted once per transcript message, so each copy guarded only itself.
 * Upgrade path: `chat-session.machine`'s `run` facet owns the turn (C3), and
 * `queued` refusing a second `requestTurn` replaces this outright.
 */
const preparingChats = new Set<string>();
...
export type TurnAdmission = Readonly<{
  ...
  /** Admit, then dispatch — at most one dispatch of this chat at a time. */
  withWorkspace: (
    turnId: string | undefined,
...
export const useTurnAdmission = (liveExecution: CadAgentExecution): TurnAdmission => {
  ...
  const withWorkspace = useCallback(
    async (
       ...
       * selection. Admission has to see the same row the body names, or the
      ...
      if (!workspaceAuthority && liveExecution.kind === 'tau' && liveExecution.hostId === undefined) {
        return;
      }
      if (preparingChats.has(activeChatId)) {
      ...
      preparingChats.add(activeChatId);
      try {
        operation(...(await admitWorkspace(turnId, turnExecution ?? liveExecution)));
      } catch (error) {
        surfaceDispatchFailure(error);
      } finally {
        preparingChats.delete(activeChatId);
      }
    },
    [activeChatId, admitWorkspace, liveExecution, surfaceDispatchFailure, workspaceAuthority],
  );

  return { admitWorkspace, surfaceDispatchFailure, withWorkspace, awaitResolvedModel };
};
