...
describe('metalMorphSpinnerService', () => {
  ...
  it('should ask the balanced tier for a supersampled source covering the largest spinner', async () => {
    const service = createMetalMorphSpinnerService();
    service.subscribe(createTarget({ size: 16 }));
    await settle();
    expect(hoisted.createOptions[0]).toMatchObject({ quality: 'balanced' });

    // 16 CSS px at a 2× ratio is 32 device px, and the source is drawn twice as fine again.
    expect(service.getDiagnostics().sourceSize).toBe(64);

    service.subscribe(createTarget({ size: 96 }));
    await settle();

    // 96 CSS px would ask for 384 the same way, which the shared source caps at 256.
    expect(service.getDiagnostics().sourceSize).toBe(256);
    expect(lastController().setSize).toHaveBeenCalledWith({ width: 256, height: 256, pixelRatio: 1 });
  });

  it('should open a renderer for a warm-up before any spinner subscribes', async () => {
    const service = createMetalMorphSpinnerService();
    await service.warm('light');

    expect(service.getDiagnostics().rendererCount).toBe(1);
    expect(hoisted.createOptions[0]).toMatchObject({ theme: 'light' });
  });

  it('should give the first spinner the warmed renderer rather than opening a second', async () => {
    const service = createMetalMorphSpinnerService();
    await service.warm('dark');
    service.subscribe(createTarget());
    await settle();

    expect(hoisted.createOptions).toHaveLength(1);
    expect(service.getDiagnostics().rendererCount).toBe(1);
  });

  it('should release a warmed renderer no spinner ever claimed', async () => {
    const service = createMetalMorphSpinnerService();
    await service.warm('dark');
