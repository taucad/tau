...
import type { MetalMorphLoaderTheme } from '#components/geometry/loader/metal-morph-controller.js';
...
/**
 * Open the shared renderer before the first spinner mounts, so it has a body on its first painted frame rather
 * than its stand-in. Repeat calls are free, and a renderer warmed for a spinner that never arrives is released
 * on the same idle delay as one whose spinners all left.
 */
export const warmMetalMorphSpinner = async (theme: MetalMorphLoaderTheme): Promise<void> => {
  await getMetalMorphSpinnerService().warm(theme);
};
