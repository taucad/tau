...
function CommandList({ className, ...properties }: React.ComponentProps<typeof CommandPrimitive.List>) {
  return (
    <CommandPrimitive.List
      ...
      className={cn('max-h-[500px] scroll-py-1 overflow-x-hidden overflow-y-auto pb-1', className)}
      {...properties}
    />
  );
}
...
function CommandItem({ className, ...properties }: React.ComponentProps<typeof CommandPrimitive.Item>) {
  return (
    <CommandPrimitive.Item
      ...
      className={cn(
        "data-[selected=true]:text-accent-foreground relative flex h-9 cursor-pointer items-center gap-1 rounded-md px-2 text-sm outline-hidden select-none data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50 data-[selected=true]:bg-accent [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 [&_svg:not([class*='text-'])]:text-muted-foreground",
        className,
      )}
      {...properties}
    />
