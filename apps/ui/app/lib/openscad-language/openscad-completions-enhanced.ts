/**
 * Enhanced OpenSCAD completions provider with tree-sitter integration
 * This provides the same API as openscad-completions.ts but uses tree-sitter when available
 */
...
} from './openscad-descriptions.js';
import { openscadSymbols, openscadFunctions, openscadConstants } from './openscad-symbols.js';
import type {
  OpenscadModuleSymbol,
  OpenscadFunctionSymbol,
  OpenscadConstantSymbol,
} from './openscad-symbols.js';
import {
...
// Note: These functions are implemented locally to avoid import issues
...
export function createEnhancedCompletionItemProvider(monaco: typeof Monaco): Monaco.languages.CompletionItemProvider {
  return {
    ...
    async provideCompletionItems(model, position) {
      ...
      if (!groupName && (await isPositionInCommentWithFallback(model, position))) {
        return { suggestions: [] };
      }
      ...
      // Check tree-sitter availability and add indicator
      const treeSitterStatus = await checkTreeSitterAvailability();
      if (treeSitterStatus.available) {
        completions.push({
          label: '🌲 Tree-sitter parsing active',
          kind: monaco.languages.CompletionItemKind.Text,
          documentation: { value: 'Enhanced parsing with tree-sitter is active' },
      ...
      // Add user-defined items using enhanced parsing
      const userDefined = await findUserDefinedItemsWithFallback(model);

      // Add debug information about parsing method
      if (userDefined.parseMethod === 'tree-sitter') {
...
// Export the original provider with a different name for backwards compatibility
export { createCompletionItemProvider as createOriginalCompletionItemProvider } from './openscad-completions.js';