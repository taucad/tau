...
type FixtureKind = 'edge' | 'gltf' | 'section-overlap' | 'svg';
...
const sectionOverlapModel3d = `import { makeBaseBox } from 'replicad';

export default function main() {
  return [
    { shape: makeBaseBox(40, 30, 24).translate([0, -40, 0]), color: '#06b6d4', name: 'Overlap cyan' },
    { shape: makeBaseBox(40, 30, 24).translate([0, -25, 0]), color: '#3b82f6', name: 'Overlap blue' },
    { shape: makeBaseBox(40, 20, 24).translate([0, 15, 0]), color: '#22c55e', name: 'Touch green' },
    { shape: makeBaseBox(40, 20, 24).translate([0, 35, 0]), color: '#a3a3a3', name: 'Touch gray' },
  ];
}
`;
  ...
  'src/main.ts': {
    content: encode(
      kind === 'svg'
        ? drawing2d
        : kind === 'edge'
          ? edgeLegibilityModel3d
          : kind === 'section-overlap'
            ? sectionOverlapModel3d
            : model3d,
    ),
  },
...
const HeadlessChatImageCaptureDebugRoute = (): React.JSX.Element => {
  ...
  React.useEffect(() => {
    ...
    const kind: FixtureKind =
      requestedKind === 'svg' || requestedKind === 'edge' || requestedKind === 'section-overlap'
        ? requestedKind
        : 'gltf';
    const seed = async (): Promise<void> => {
      try {
        const project = await createProject({
          ...
          activeExecution: { kind: 'tau', model: 'e2e-image-model' },
