...
export function ConverterFileTree({ selectedFormats, fileName }: ConverterFileTreeProps): React.ReactNode {
  ...
  // Multiple files - show as zip with tree
          ...
          <div className="mt-1 ml-2 space-y-1 border-l border-border pl-2">
            {files.map((file, index) => (
              <div key={file} className="flex items-center gap-1.5 text-xs">
                <div
                  className={`h-px w-2 bg-border ${index === files.length - 1 ? 'rounded-r' : ''}`}
                  style={{
                    marginLeft: '-0.5rem',
                  }}
                />
