...
import { ChatHistory, ChatHistoryTrigger } from '#routes/w.$workspace.$project/chat-history.js';
import { ChatHistoryGate, ChatInterfaceSessionGate } from '#routes/w.$workspace.$project/focused-chat-gate.js';
import { ChatFileTree, ChatFileTreeTrigger } from '#routes/w.$workspace.$project/chat-file-tree.js';
import { ChatParameters, ChatParametersTrigger } from '#routes/w.$workspace.$project/chat-parameters.js';
import { ViewerDockview } from '#routes/w.$workspace.$project/chat-viewer-dockview.js';
import { ChatEditorLayout, ChatEditorLayoutTrigger } from '#routes/w.$workspace.$project/chat-editor-layout.js';
import { ChatExplorerTree, ChatExplorerTrigger } from '#routes/w.$workspace.$project/chat-explorer.js';
import { ChatKernel, ChatKernelTrigger } from '#routes/w.$workspace.$project/chat-kernel.js';
import { ChatDetails, ChatDetailsTrigger } from '#routes/w.$workspace.$project/chat-details.js';
import { ChatConverter, ChatConverterTrigger } from '#routes/w.$workspace.$project/chat-converter.js';
...
export const ChatInterfaceDesktop = memo(function (): React.JSX.Element {
  ...
  return (
    <ChatInterfaceSessionGate fallback={<div className='hidden size-full md:flex' />}>
      <ChatContextInsertionProvider>
        <SidebarOffset via='padding' className='size-full pt-(--header-height) pb-2'>
          <div
            ref={allotmentRef}
            className='size-full'
          ...
          >
            ...
            >
              ...
              >
                {/* Top-left Content */}
                <div
                  className={cn('absolute top-10 z-10 flex flex-col gap-2', isAnyLeftPanelOpen ? 'left-2' : 'left-4')}
                >
                  <ChatHistoryTrigger
                    isOpen={isChatOpen}
                    onToggle={() => {
