...
import * as observabilityModule from '#runtime/observability/observability.middleware.js';

const { observabilityMiddleware } = observabilityModule;
...
describe('observability middleware registration', () => {
  ...
  it('passes reportUrl option through to plugin options', () => {
    const plugin = observabilityMiddleware({ reportUrl: 'https://api.test/ingest' });
