...
describe('headless capture adapter', () => {
  ...
  it('keeps agent edge intent while scaling lines in final-output pixels', async () => {
    ...
    expect(exportImage.mock.calls[0]![0]).toMatchObject({
      includeEdges: false,
      exportOptions: { width: 1600, height: 1600, lineWidth: 2 },
    });
  });

  it('keeps the output-pixel edge recipe explicit for utility captures', async () => {
    ...
    expect(exportImage.mock.calls[0]![0]).toMatchObject({
      exportOptions: { width: 2400, height: 1350, lineWidth: 2400 / 800 },
    });
  });
