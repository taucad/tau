...
Configure the esbuild bundler for JS/TS/TSX kernels (Replicad, JSCAD, TSCircuit). The bundler resolves imports, bundles code, and executes it inside the worker. You can customize extensions, and for advanced use cases, define custom bundlers.
...
Pass the bundler in the `bundlers` array. JS-family kernels (Replicad, JSCAD, TSCircuit) require a bundler to resolve imports and execute code:
...
The flow for Replicad, JSCAD, and TSCircuit:
...
See the Replicad, JSCAD, and TSCircuit kernel source for full examples.
