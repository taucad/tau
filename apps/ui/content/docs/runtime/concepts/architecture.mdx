...
**Main Thread** -- display and user input. Houses the editor, Three.js renderer, parameter UI, and progress indicators. The editor writes files to the File Manager Worker through a validated bridge proxy (`createFileSystemBridge` + `createFileSystemBridgeProxy`). The `RuntimeClient` sends `render`/`updateParameters`/`setOptions`/`export` commands to the kernel worker and subscribes to geometry, state, and progress events.
...
| ------------------ | -------------------------------- | ---------------------------------------------------------- |
| `render`           | User opens file                  | Render immediately, discover deps, start watching          |
...
The main thread sends `render` and `updateParameters`. File edits flow through the filesystem bridge. Everything else -- scheduling, dependency watching, cache invalidation, abort, and result delivery -- happens inside the worker.
...
The [`RuntimeClient`](../api/client) is the consumer-facing API. `createRuntimeClient(options)` returns a client whose transport handle is materialized during construction (`TransportPlugin.materialize()`); worker spawn and the transport handshake remain lazy until `connect()` or an auto-connect command (`render` / `export`). Inline `source.files` input is staged into the transport-owned filesystem. High-level helpers provide that filesystem automatically; raw transports require `fileSystem`. Hosts that own a filesystem supply a wired transport plugin at construction (`transport: webWorkerTransport({ fileSystem: fromFileSystemBridge(openConnection) })`) and call `client.connect()` (no arguments); see [Embedding in a Host](../guides/embedding-in-a-host). For persisted projects, `openConnection` returns a fresh writable rooted bridge. A [runtime path](./path-namespaces) is local to that supplied filesystem, so its leading `/` is not the host operating system's root. `client.on(event, handler)` consumes the event stream, and the active transport owns the abort channel and `postMessage` event delivery — the client itself never touches `SharedArrayBuffer` or `Atomics`. See [Client API](../api/client) for the full reference.
...
- **Bundler** -- lazy-loaded for JS/TS kernels. Bundles entry paths and dependencies, executes code.
- **Autonomous render loop** -- `render` renders immediately; file changes debounce at 500ms; parameter changes debounce at 50ms. See [Render Lifecycle](./render-lifecycle).
...
- **Editor and FileSystem**: The editor writes files to the File Manager Worker through Bridge A (`createFileSystemBridgeProxy`). This is the write path that triggers autonomous re-renders. The main thread never writes files through the kernel worker.
...
- **Worker and FileSystem**: The kernel worker accesses the filesystem through Bridge B. In autonomous mode, it subscribes to file change events scoped to the current file's dependency graph. Watch events flow directly from the File Manager Worker -- they never pass through the main thread. With an inline Node filesystem (`fromNodeFs`) there is no File Manager Worker: the adapter watches the host directories and delivers the same events in-isolate.
