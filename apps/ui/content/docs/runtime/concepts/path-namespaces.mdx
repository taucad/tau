...
A **runtime path** identifies a file within the filesystem capability supplied to one runtime client. It is a canonical POSIX path relative to that capability: `main.ts`, `models/main.ts`, or `''` for the capability root. It never has a leading slash.
...
Filesystem adapters establish which resource becomes the capability root:
...
- [Filesystem API](../api/filesystem) -- constructors that establish a runtime filesystem root
