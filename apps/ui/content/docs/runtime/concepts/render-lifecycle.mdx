...
The runtime worker manages its own render lifecycle autonomously. After receiving `render`, it renders immediately, discovers dependencies, watches for changes, and re-renders on file or parameter updates. A watched-file rerender can therefore supersede the originating public call without a second API call. Every admitted preview reaches `idle` or `error` before its successor becomes active, and selected geometry is published over the `geometry` event. Three cancellation strategies ensure that superseded renders are aborted as quickly as possible while guaranteeing correctness.
...
**render(source, parameters):** Normalize the source entry to a [runtime path](./path-namespaces), store it with the parameters, and render immediately (aborting any in-progress render). After rendering, discover the entry's dependencies. Set up a filesystem watch subscription scoped to the dependency set. Push `geometryComputed` to the main thread.
...
- **Dependencies and Watch**: `getDependencies` returns runtime paths that the worker subscribes to via the filesystem bridge. Watch events use that same namespace and arrive directly from the File Manager Worker via MessagePort. Changes to watched files trigger re-renders.
...
- **OpenRSCAD limitation** -- OpenRSCAD evaluates a model synchronously inside its worker. The generation counter handles correctness, but the current evaluation must complete before a newer render begins.
