...
```mermaid
flowchart TD
    Render["render(file, params)"] --> Render1["render immediately"]
...
**render(file, params):** Store the file and params. Render immediately (aborting any in-progress render). After rendering, discover the file's dependencies. Set up a filesystem watch subscription scoped to the dependency set. Push `geometryComputed` to the main thread.
