...
After receiving `render`, the worker manages its own render lifecycle:

1. **render(file, params)** -- Store file and params. Render immediately (aborting any in-progress render). Discover dependencies. Set up filesystem watch subscription. Push `geometryComputed`.
...
**Bridge B (kernel worker):** The active [transport](../api/client) owns the kernel-worker bridge end-to-end. Consumers pass an opaque `RuntimeFileSystem` to the transport at construction — `fromFileSystemBridge(openFileSystemBridge(fsWorker))` for the worker-backed arm, `fromMemoryFs()`/`fromFsLike()`/`fromNodeFs()`/`fromBrowserFs()` for inline arms — and the transport transfers the filesystem `MessagePort` to the kernel worker as part of `initialize`. The kernel worker creates a bridge proxy for file reads, dependency resolution, and watch subscriptions. Bridge connections are disposed with the runtime session that opened them.
...
A [runtime path](./path-namespaces) identifies a file within the filesystem supplied to the client. Normalized worker-side paths begin with `/`, but that slash names the supplied filesystem's root rather than the host operating system's root. Runtime receives neither authority-global `/projects/<id>` paths nor host filesystem paths.
...
const fsBridge = openFileSystemBridge(fmWorker);
...
The worker entry composes `defineRuntime(...)`, `createRuntimeWorker({ runtime })`, and **`webWorkerHost`**.

If the runtime definition is config-backed, the same topology passes boot config on the client:
...
// under the hood: presets.all() + inProcessTransport({ runtime, fileSystem: fromNodeFs('/path/to/project') })
```
...
/** Electron apps supply a wired transport plugin (e.g. app-local `electronUtilityTransport` — see `examples/electron`). */
export function createElectronRendererClient(transport: TransportPlugin): ReturnType<typeof createRuntimeClient> {
  return createRuntimeClient<typeof runtime>({
    transport,
  });
}
```

On the main / utility-process side, pair this with `createRuntimeHost` from `@taucad/runtime/host` and the worker transport host factory that matches your topology (`nodeWorkerHost` wiring lives next to the Node worker entry — follow `examples/electron` for a complete IPC host).
