...
`createRuntimeClient` returns a high-level client that manages worker lifecycle, kernel selection, and middleware. Inline `source.files` input is staged into the transport-owned filesystem, and `source.entry` selects a key from that map. Raw transports need `fileSystem`, while high-level helpers provide one automatically. See [Path Namespaces](../concepts/path-namespaces) for the relationship between consumer source paths and normalized plugin entry paths.
...
Hand the model code, entry path, and parameters to `client.export('glb', input)` -- this self-renders the model and returns the binary glTF in one call:
...
`export(format, input)` self-renders the input and resolves an ordered, non-empty
artifact array -- the `success` discriminant narrows the payload so you can
safely iterate `result.data` in the `true` branch. If rendering fails, the
...
console.log(`Rendered ${result.success ? result.data.reduce((sum, file) => sum + file.bytes.byteLength, 0) : 0} bytes`);
