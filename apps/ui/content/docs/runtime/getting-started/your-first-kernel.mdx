...
`createRuntimeClient` returns a high-level client that manages worker lifecycle, kernel selection, and middleware. Inline `source.files` input is staged into the transport-owned filesystem; raw transports need `fileSystem`, while high-level helpers provide one automatically.
