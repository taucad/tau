...
Seed keys and mock `entryPath` values as [runtime paths](../concepts/path-namespaces). A leading `/` refers to the test filesystem's root.
...
describe('my kernel', () => {
  ...
  it('runs against the seeded filesystem', () => {
...
it('passes geometry through unchanged', async () => {
  ...
  const input = createMockInput({ entryPath: '/main.ts' });
...
it('produces a valid GLB with the expected geometry', async () => {
  const worker = await createTestWorker(replicad, {
    'main.ts': `
      import { drawRoundedRectangle } from 'replicad';
      export default function main() {
...
import { createTestWorker, createGeometryFile, createGeometryTestHelpers } from '@taucad/runtime/testing';
import { replicad } from '@taucad/runtime/kernels/replicad';

it('matches expected mesh, vertex count, and bounding box', async () => {
  const worker = await createTestWorker(replicad, {
    'main.ts': `
      import { drawRoundedRectangle } from 'replicad';
      export default function main() {
