...
| `STRIPE_SECRET_KEY`, `STRIPE_READ_SECRET_KEY`                  | Separate restricted collection and read credentials required only in Cloud mode. Test mode requires distinct `rk_test_…` keys; live mode requires distinct `rk_live_…` keys. Full `sk_…` keys are rejected. |
