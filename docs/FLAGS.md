...
Operator reference for first-party API, UI, desktop and `tau serve` flags, provider credentials and related configuration. **Status: 2026-09-15 source inventory plus ratified billing-closeout targets. `TAU_CLOUD_ENABLED` is not implemented in the current code.** The [closeout](research/tau-cloud-credit-billing-launch-charter.md) must update this reference and prove both profiles before marking it operational.

## Deployment mode

| Flag                                                   | Target default / values                       | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ------------------------------------------------------ | --------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `TAU_CLOUD_ENABLED`                                    | `false`; strict `true`/`false`                | **Ratified target.** The single API/UI presence switch for Tau Cloud billing and funded LLM admission. At API bootstrap false excludes cloud-only Nest modules/services. At UI build time false excludes cloud-only modules/assets from all output, including lazy/worker/SSR chunks; shared boundaries are side-effect-free no-ops. Operator-configured providers run without Tau credit checks. True retains all financial gates. |
...
Set the same cloud mode in API bootstrap and UI build configuration. **Changing this flag requires rebuilding the UI and restarting the API.** Desktop/serve must select the matching prebuilt renderer; `window.ENV`, preload and browser storage cannot override compiled mode. The API remains authoritative for charging. URLs and other public runtime settings can still be injected. Never expose provider keys in `window.ENV`. Production cloud manifests must explicitly assert true so a missing deployment value cannot accidentally serve unbilled calls.

**Target self-host profile:** cloud=false, own URLs, normal application database/Redis/auth/storage, and only credentials for providers you use. **Target cloud profile:** cloud=true plus coherent financial environment, Stripe account/mode, billing secrets, published policy and funded supplier routes. These profiles require CL01–CL04; current production API validation still demands billing configuration.
...
| -------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `OPENAI_API_KEY`                                   | OpenAI Responses provider. Currently schema-required; target optional unless selected. Title/commit/completion currently assume OpenAI models; closeout must resolve one-provider installations consistently.                |
| `ANTHROPIC_API_KEY`                                | Anthropic Messages provider. Currently schema-required; target required only when configured.                                                                                                                                |
| `GOOGLE_VERTEX_AI_CREDENTIALS`                     | Service-account JSON object, including project/client email/private key. Currently schema-required; target optional unless Vertex is configured. Never serialize it to clients.                                              |
| `TOGETHER_API_KEY`, `MORPH_API_KEY`, `XAI_API_KEY` | Optional keys with current hosted adapter families. Model availability must agree with actual supported routes and configured credentials.                                                                                   |
| `CEREBRAS_API_KEY`, `MOONSHOT_API_KEY`             | Optional schema entries; not in the six-family funded adapter map. Do not assume a usable gateway route merely because a key is accepted. Final closeout must document/remove stale exposure or qualify the advertised path. |
| `TAVILY_API_KEY`                                   | Optional search-provider credential; not an LLM credit-mode switch.                                                                                                                                                          |
| `OLLAMA_ENABLED`                                   | API local-model discovery; default false. Current schema uses boolean coercion: omit it to disable reliably; the string `false` is not a safe off value until CL01 fixes parsing.                                            |
...
| `TAU_PROVIDER_DIAGNOSTICS_VERBOSE` | API boolean coercion, default false                            | Sanitized success-path provider diagnostics. Failures remain logged. Closeout must fix string-false behavior.                                                        |
...
| `TAU_S3_FORCE_PATH_STYLE`          | Boolean coercion, default true                                 | S3 addressing for local/object storage. Target consistent strict parsing; do not assume string `false` currently disables it.                                        |
...
| `STRIPE_SECRET_KEY`, `STRIPE_READ_SECRET_KEY`                  | Collection and authoritative-source access; server secrets. Empty keys currently refuse collection but are **not** the intended self-host switch.      |
...
| `TAU_HOST_MODEL`, `TAU_HOST_MODEL_PROVIDER`                                                 | Default catalog model and provider (`openai-gpt-5.6-luna`, `openai` currently).                                  |
...
CL01–CL04 and CL10 must replace target labels with verified behavior, document final defaults/parser changes, update examples/host allowlists and run production cloud-on/off builds. Prove cloud Nest providers/lifecycle are absent, cloud-only files/modules/manifests are absent from all UI build outputs, and no billing network/SQL/UI activity occurs off while normal providers execute; prove paid enforcement on. See the [boundary contract](research/artifacts/tau-cloud-credit-billing-launch-charter/runs/implementation/billing-cloud-boundary.md). New user-facing flags belong here with values, default, consumer, secret/public status and restart/build behavior.
