...
| [SIMD128 / `v128`](https://developer.mozilla.org/en-US/docs/WebAssembly/Reference/SIMD)                                                              | Required for universal SIMD artifacts                  | OCJS, Replicad, libassimp                                      | ✅                      | ✅                               | ✅                                        | ✅                                  | Statically inspect emitted features and instantiate the shipped artifact         | No scalar substitution inside one artifact; ship a separately qualified artifact if ever needed           | 2026-07-21 |
...
| Electron                   | Runtime peer range and first-party applications                        | `@taucad/runtime` peer `>=37.0.0`; `apps/desktop` and both examples pin `^43.5.1`        | Package, example, and desktop-shell smoke tests on the pinned Electron                    | Report the unsupported peer/host; do not assume Chrome-stable parity                                 | 2026-09-02           |
...
| Tau Host relay             | `@taucad/host`, CLI, API relay, and browser matrix                     | Experimental trusted-project cohort; same Tau build; one API process                     | Exact GLB parity in Chromium, Firefox, and automated WebKit; branded Safari lane separate | Visible offline/busy/skew/disconnected state; public promotion blocked on sandbox and branded Safari | 2026-08-28 (partial) |
