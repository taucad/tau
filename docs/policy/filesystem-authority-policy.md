...
| PP3 | **Control plane matches a folded segment, never a root**: a `control-plane` row matches wherever its name appears as a path segment, compared lower-cased and without trailing dots or spaces — every desktop filesystem folds those onto the same directory — so the answer depends on neither how deep the repository sits nor how it was spelled                                       |
