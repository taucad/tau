...
| Archive resident input bytes                                        | one file at a time; default ceiling 256 MiB | one file at a time; refuses before exceeding the ceiling by more than the crossing file | `libs/filesystem/src/content-ops/index.test.ts` — `should refuse an archive over its byte ceiling instead of reading the whole tree`                                                                        |
