...
This example shows `@taucad/runtime` running in a Next.js 16 app with Turbopack and an app-owned module worker. The UI uses `@taucad/react` as the happy path, while executable CAD capabilities stay in the worker.

## Commands

```bash
pnpm nx dev example-nextjs
...
```mermaid
flowchart LR
  UI["Next.js client component"] --> Hook["@taucad/react useRuntime"]
...
- The browser component imports `tau/runtime-definition.ts` as a type-only witness.
