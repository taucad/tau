...
Use resolutionMode "declared-only" when inferred semantics are not acceptable. The result retains native values, semantic bindings, provenance, assumptions, diagnostics, and the exact identity required by apply_parameter_operation. Reading in the same mode never disturbs a pending source-unit plan; reading in a different mode rejects it. An "unresolved" result carries a diagnostic code: INVALID_RECORD or UNSUPPORTED_RECORD means the saved values cannot be read and are preserved untouched, and RESOLUTION_SUPERSEDED means a concurrent read changed the mode.`,
