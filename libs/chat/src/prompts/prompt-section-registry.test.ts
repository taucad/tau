...
describe('createSectionRegistry', () => {
  ...
  describe('onSectionResolved telemetry callback', () => {
    ...
    it('should resolve current section values and emit telemetry on each call', () => {
      const registry = createSectionRegistry();
      let value = 'FIRST';
      registry.register({ name: 'role', compute: () => value, cacheBreak: false });

      const onSectionResolved = vi.fn();
      expect(registry.resolve({ onSectionResolved }).static).toBe('FIRST');
      value = 'SECOND';
      expect(registry.resolve({ onSectionResolved }).static).toBe('SECOND');
