...
export const globSearchInputSchema = z.object({
  ...
  path: rootedPathSchema.optional().describe('The base directory to search from. Defaults to project root.'),
});

const baseGlobEntrySchema = z.object({
  path: rootedPathSchema.describe('The matched path.'),
...
export const globSearchOutputSchema = z.object({
  files: z.array(rootedPathSchema).describe('The list of file paths matching the glob pattern.'),
