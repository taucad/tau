...
function profileBox(
  inputProfile: Drawing,
  base: Drawing,
) {
  const start =
    inputProfile.blueprint.firstPoint;
  const profile =
    inputProfile.translate(
      -start[0],
      -start[1],
    );
  const end =
    profile.blueprint.lastPoint;
  const baseSketch =
    base.sketchOnPlane() as Sketch;

  // We create the side of the box
  const side = baseSketch
    .clone()
    .sweepSketch(
      (plane: Plane): Sketch => {
        return profile.sketchOnPlane(
          plane,
        ) as Sketch;
      },
      {
