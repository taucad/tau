...
const main = async (): Promise<void> => {
  ...
  const document100 = createBoxDocument(boxes100);
  const doc1000 = createBoxDocument(boxes1000);
  const subject100 = await loadSubjectFromDocument(document100, 'boxes-100');
  ...
  const nativeModule = (await nativeFactory()) as unknown as GeoSpecNativeStepBackend;
  ...
  const samples = [
    await time('loadMesh GLB 100 boxes', iterations, async () =>
      subjectSummary(await loadSubjectFromDocument(document100, 'boxes-100-load')),
    ),
    await time('loadMesh GLB 1000 boxes', Math.max(3, Math.ceil(iterations / 2)), async () =>
    ...
    await time('loadStep cube shared module mesh true', Math.max(3, Math.ceil(iterations / 2)), async () =>
      loadStep({ source: stepPath, openCascade: nativeModule, mesh: true }).then(subjectSummary),
    ),
    await time('loadStep cube shared module mesh false', Math.max(3, Math.ceil(iterations / 2)), async () =>
