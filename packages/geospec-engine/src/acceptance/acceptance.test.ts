 ...
 * (`#proofs/index.js`). The proof surface is mandatory: a missing engine
 * module is a collection failure, never a skipped acceptance row.
 *
...
const proofContext = (loaded: LoadedFixture): Parameters<typeof proveRelationship>[0]['context'] => ({
...
const assertRelationshipExpectation = (loaded: LoadedFixture, row: FixtureRelationshipExpectation): void => {
  ...
  const evidence = proveRelationship({ subject, target, expectation, context: proofContext(loaded) });
...
describe('GeoSpec fixture acceptance harness', () => {
  ...
  for (const entry of fixtures) {
    ...
    describe(manifest.fixture, () => {
      ...
      for (const row of manifest.relationships ?? []) {
        ...
        it(`should prove ${row.kind} '${subjectLabel}' → '${targetLabel}' as '${row.expected.verdict}'`, () => {
  ...
  describe('matcher preconditions (master cases 5 and 8)', () => {
    it('should yield no proof context for a mesh-only subject — the D5 matcher precondition input (master case 5)', async () => {
      ...
      expect(getSubjectProofContext(meshResult.subject)).toBeUndefined();
    });

    it('should reject an explicit fixture selector in a production relationship (master case 8)', () => {
  ...
  describe('manifest schema validation', () => {
    ...
    it('should reject non-object relationship rows', () => {
      expect(() => parseFixtureManifest({ ...valid, relationships: ['contact'] }, 'bad.json')).toThrow(
        'relationships[0] must be an object',
      );
    });

    it('should reject parked relationship rows', () => {
      expect(() =>
        parseFixtureManifest(
          { ...valid, relationships: [{ kind: 'contact', subject: 'a', target: 'b', pending: 'later' }] },
          'bad.json',
        ),
      ).toThrow('must not carry a pending marker');
    });
  });
});
