...
// Native fixtures: valve-stem-guide (a 20x20x45 box with a single r4.03 +Z
// through-bore, exact lumen ~51 mm2) and filter-inside-housing (a cup with a
// blind r27 cavity over a 3 mm floor).
...
describe('canonical void-topology engine', () => {
  ...
  ): GeometryDiagnostic[] => proveVoidContinuity(expectation, context);

  it('should pass a connected through-void', () => {
  ...
  it('should fail a broken path across the sealing floor via mesh Decompose', () => {
    const diagnostics = prove(
      {
        path: [
          [0, 0, 30],
    ...
    expect((diagnostics[0]?.details as { engine?: string } | undefined)?.engine).toBe('topological');
    expect(diagnostics[0]?.spatial?.center).toBeDefined();
  }, 30_000);

  it('should fail a waypoint buried in the floor material', () => {
    const diagnostics = prove(
      {
        path: [
          [0, 0, 30],
  ...
  it('should pass isolation holding across the floor', () => {
    const diagnostics = prove(
      {
        path: [
          [0, 0, 10],
  ...
  it('should fail isolation breached within one cavity component', () => {
    const diagnostics = prove(
      {
        path: [
          [0, 0, 10],
  ...
  it('should narrow the cross-section to the bore lumen, not the whole component', () => {
    // The measured throat must be the r4.03 bore (~51 mm2), proving
    // CrossSection.decompose() + point-in-polygon
  ...
  it('should pass a wide-enough lumen clearing its minimum cross-section', () => {
    const diagnostics = prove(
      {
        path: [
          [0, 0, 5],
  ...
  it('should measure the bore throat within 1% of analytic, conservatively and deterministically (§16)', () => {
    ...
    const readMeasured = (diagnostics: GeometryDiagnostic[]): number =>
      (diagnostics[0]?.details as { measuredCrossSection?: number } | undefined)?.measuredCrossSection ?? 0;
    const first = readMeasured(proveVoidContinuity(claim, guideContext));
    const second = readMeasured(proveVoidContinuity(claim, guideContext));
  ...
  it('should emit the CR1 constructibility census under forensic timing', () => {
    ...
    const observedContext: RelationshipProofContext = {
      ...guideContext,
      forensic: measurements.push.bind(measurements),
    };
    const connectivity = proveVoidContinuity(
      {
        path: [
          [0, 0, 3],
          [0, 0, 42],
    ...
    const throat = proveVoidContinuity(
      {
        path: [
          [0, 0, 5],
          [0, 0, 40],
    ...
    const census = measurements.filter(({ name }) => name.startsWith('void.census.'));
    const read = (label: string): number[] =>
      census.filter(({ name }) => name === `void.census.${label}`).map(({ value }) => value);
    ...
    // The derived region is the material's padded bounds, so the single
  ...
  it('should reject a claim with neither material nor bounds', () => {
    ...
    const diagnostics = proveVoidContinuity(expectation, housingContext);
...
describe('V4 arbitrary-axis cross-section', () => {
  it('should measure the true perpendicular throat of an oblique bore', async () => {
    ...
    const result = lumenBottleneck({ solid: oblique, waypoints: [from, to] });
    oblique.delete();
...
describe('§16 cross-section determinism and tessellation bias', () => {
  it('should under-report the true section (conservative), converge with tessellation, deterministically', async () => {
    ...
    const measure = (segments: number): number => {
      ...
      const first = lumenBottleneck({ solid: cylinder, waypoints: [from, to] });
      const second = lumenBottleneck({ solid: cylinder, waypoints: [from, to] });
      cylinder.delete();
...
describe('void-topology shell persistence (R5)', () => {
  ...
  it('should replay decomposed shells with zero tessellation fetches on the warm run', () => {
    ...
    const cold = proveVoidContinuity(claim, counting);
    ...
    const warm = proveVoidContinuity(claim, counting);
