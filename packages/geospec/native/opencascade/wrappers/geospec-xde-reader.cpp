...
// Forensic phase timing, mirroring packages/geospec/src/runner/forensic.ts.
// Enabled per read via {"forensic":true} in the XDE reader optionsJson (set
// by load-step.ts when GEOSPEC_FORENSIC is set); emits
// "[FORENSIC] <label>\t<value>" on stderr. The module is single-threaded, so
// a file-scope flag reset per read is safe.
// Weak external linkage, deliberately (wrapper-hygiene rule, blueprint C2):
// the ocjs binder compiles every bound symbol as its own translation unit
// containing ALL wrapper sources, so a strong global is a duplicate symbol at
// link and a static is silently per-TU (a facet method bound through one
// symbol would never observe a flag written through another). A weak
// definition merges to one runtime instance shared by every TU.
__attribute__((weak)) bool geospecForensicEnabled = false;

static double geospecForensicNowMs() {
  return std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now().time_since_epoch()).count();
}

static void geospecForensicLog(const char* label, double startMs) {
  if (geospecForensicEnabled) {
    std::cerr << "[FORENSIC] " << label << "\t" << (geospecForensicNowMs() - startMs) << "\n";
  }
}

static void geospecForensicValue(const char* label, double value) {
  if (geospecForensicEnabled) {
    std::cerr << "[FORENSIC] " << label << "\t" << value << "\n";
  }
}
...
public:
  ...
  // for the hybrid void-occupancy engine (geospec-policy sections 17/19/20:
  // mesh-boolean bulk occupancy, exact classification reserved for the
  // boundary band). Reports the achieved deflection floored at the requested
  ...
  // Tessellates ONE face of a placed occurrence (same winding/location and
  // deflection contract as occurrenceMeshTriangles) into the retained triangle
  // buffer — the exact trimmed per-face footprint for the topological
  // contact-patch engine (spatial-relationship blueprint R1). `face` is the
  // same 0-based TopExp_Explorer(placed, TopAbs_FACE) ordinal faceFacts and
  // extrema use, so a face index means the same face across every proof facet.
  // Clobbers the root meshTriangles buffer by design: consumers copy out
  // immediately (the same transfer contract as meshTriangles).
  std::string occurrenceFaceMeshTriangles(int occurrence, int face, const std::string& optionsJson) {
    try {
      const double start = geospecForensicNowMs();
      std::string error;
      const TopoDS_Shape shape = resolveProofShape(occurrence, face, error);
      if (!error.empty()) return geospecXdeErrorJson(error);
      const GeoSpecStepOptions options = geospecParseOptions(optionsJson);
      double* triangleSoup = nullptr;
      int triangleCount = 0;
      ...
      json << std::setprecision(17);
      json << "{\"triangleCount\":" << meshTriangleCount_ << ",\"deflection\":" << achievedDeflection << "}";
      geospecForensicLog("native.facet.occurrenceFaceMeshTriangles", start);
      return json.str();
    } catch (const std::exception& error) {
      return geospecXdeErrorJson(error.what());
    } catch (...) {
      return geospecXdeErrorJson("GeoSpec XDE occurrenceFaceMeshTriangles failed with an unknown native error.");
    }
  }
...
public:
  static GeoSpecXdeReadResult readText(const std::string& data, const std::string& optionsJson) {
    try {
      // The XDE read is the single native read per load; it owns the
      // per-read forensic flag ({"forensic":true} from load-step.ts).
      geospecForensicEnabled = optionsJson.find("\"forensic\":true") != std::string::npos;
