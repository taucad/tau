...
import { openrscad, openrscadExportSchemas, openrscadRenderSchema } from '#openrscad.kernel.js';
...
describe('OpenRSCADKernel', () => {
  ...
  it('preserves tessellation authored in the model unless Tau explicitly overrides it', async () => {
    const definition = await resolveRuntimePluginDefinition('kernel', openrscad());
...
`,
  ...
  it('matches the real greenhouse hierarchy and isolates the complete roof frame', async () => {
    const fixtureUrl = new URL('../test/fixtures/greenhouse/', import.meta.url);
  ...
  it('draws each edge-topology fixture exactly as its defect requires', async () => {
    const fixtureUrl = new URL('../test/fixtures/edge-topology/', import.meta.url);
  ...
  it('matches the complete planetary gearbox part manifest', async () => {
    const fixtureUrl = new URL('../test/fixtures/planetary-gearbox/', import.meta.url);
    const fixtureFiles = [
      'main.scad',
      'lib/params.scad',
      'lib/gear.scad',
