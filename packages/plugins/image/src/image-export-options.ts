...
const worldSchema = z
  .object({
    up: worldAxisSchema.default('+z'),
    forward: worldAxisSchema.default('-y'),
    unit: z.enum(['meter', 'millimeter']).default('meter'),
  })
  .strict()
  .superRefine((world, context) => {
    const vector = (axis: z.output<typeof worldAxisSchema>): readonly [number, number, number] => {
      const sign = axis.startsWith('+') ? 1 : -1;
      return axis.endsWith('x') ? [sign, 0, 0] : axis.endsWith('y') ? [0, sign, 0] : [0, 0, sign];
    };
    if (world.up[1] === world.forward[1]) {
      context.addIssue({ code: 'custom', path: ['forward'], message: 'Up and forward must use distinct axes' });
      return;
    }
    const up = vector(world.up);
    const forward = vector(world.forward);
    const remaining = ['x', 'y', 'z'].find((axis) => axis !== world.up[1] && axis !== world.forward[1]);
    const remainingVector = vector(`+${remaining}` as z.output<typeof worldAxisSchema>);
    const determinant =
      remainingVector[0] * (up[1] * forward[2] - up[2] * forward[1]) +
      remainingVector[1] * (up[2] * forward[0] - up[0] * forward[2]) +
      remainingVector[2] * (up[0] * forward[1] - up[1] * forward[0]);
    if (determinant !== 1) {
      context.addIssue({
        code: 'custom',
        path: ['forward'],
        message: 'Up and forward must define a right-handed frame',
      });
    }
  });
...
const primitiveReferenceSchema = z
  .object({
    nodeIndex: z.number().int().nonnegative().max(Number.MAX_SAFE_INTEGER).default(0),
    meshIndex: z.number().int().nonnegative().max(Number.MAX_SAFE_INTEGER).default(0),
    primitiveIndex: z.number().int().nonnegative().max(Number.MAX_SAFE_INTEGER).default(0),
  })
  .strict()
  .default({ nodeIndex: 0, meshIndex: 0, primitiveIndex: 0 })
  .meta({
    default: { nodeIndex: 0, meshIndex: 0, primitiveIndex: 0 },
    required: ['nodeIndex', 'meshIndex', 'primitiveIndex'],
  });
...
const sectionPlaneSchema = z
  .object({
    point: cameraVectorSchema.default([0, 0, 0]),
    normal: nonZeroCameraVectorSchema.default([0, 0, 1]),
  })
  .strict();
...
const orthographicFitProjectionSchema = z
  .object({ kind: z.literal('orthographic') })
  .strict()
  .meta({ title: 'Orthographic' });
const perspectiveFixedProjectionSchema = perspectiveFitProjectionSchema
  .extend({ zoom: imageZoomSchema.default(1) })
  .meta({ title: 'Perspective' });
const orthographicFixedProjectionSchema = z
  .object({
...
const fitCameraSchema = z
  ...
  .strict()
  .meta({ title: 'Fit' });
const boundsCameraSchema = fitCameraSchema.extend({ framing: z.literal('bounds') }).meta({ title: 'Bounds' });
const fixedCameraSchema = z
  .object({
    framing: z.literal('fixed'),
    position: cameraVectorSchema.default([3, -3, 2]),
    target: cameraVectorSchema.default([0, 0, 0]),
    up: nonZeroCameraVectorSchema.default([0, 0, 1]),
    projection: z
      .discriminatedUnion('kind', [perspectiveFixedProjectionSchema, orthographicFixedProjectionSchema])
...
const createImageSchema = <const SharedShape extends z.ZodRawShape, const ViewShape extends z.ZodRawShape>({
  ...
  const imageViewsSchema = z
    ...
    .default(() => [imageViewSchema.parse({ id: 'view-1' })]);
  const single = z
    .object({
      ...commonShape,
