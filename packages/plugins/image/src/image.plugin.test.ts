...
describe('@taucad/image', () => {
  ...
  it('exports the named plugin', async () => {
    ...
    expect(capabilities.transcoders.map(({ id }) => id)).toEqual(['image', 'svg-image']);
    expect(backendMock.load).not.toHaveBeenCalled();
  });
