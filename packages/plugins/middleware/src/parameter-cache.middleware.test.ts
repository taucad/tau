...
import { parameterCacheWithCache } from '#parameter-cache.middleware.js';
...
describe('parameterCacheMiddleware', () => {
  const resolveParameterCacheMiddleware = async () =>
    resolveRuntimePluginDefinition(
      'middleware',
      parameterCacheWithCache(new LruMap<GetParametersResult>({ maxEntries: 50 })),
    );
  let parameterCacheMiddleware: Awaited<ReturnType<typeof resolveParameterCacheMiddleware>>;

  // Each registration owns its L1 cache, so a fresh middleware per test is the isolation.
  beforeEach(async () => {
