...
import { activateOccParallelism } from '@taucad/occt-core';
import { inProcessTransport } from '@taucad/runtime/transport/in-process';
import { defineRuntime } from '@taucad/runtime/worker';
import { opencascadeKernel } from '@taucad/opencascade';
import { createIncrementalMesh, meshShapesToGltf } from '#opencascade-mesh.js';
...
const createOpenCascadeKernelContext = async (variant: string): Promise<BenchContext> => {
  ...
  const runtime = defineRuntime({
    kernels: [opencascadeKernel({ wasm: variant === 'multi' ? 'multi' : 'full', ocTracing: 'off' })],
...
const main = async (): Promise<void> => {
  ...
  if (values.help) {
    console.log(`
OpenCascade BRep Surface Benchmark

Usage:
  pnpm nx benchmark-brep-surfaces opencascade [-- options]

Options:
  -i, --iterations <n>   Timed iterations per case (default: 5)
  -w, --warmup <n>      Warmup iterations per case (default: 2)
  -v, --variant <name>  WASM variant (default: multi)
  -o, --output <dir>    Output directory
                        (default: ../../../out/reports/benchmarks/opencascade-brep-surfaces)
      --filter <items>  Comma-separated case or category filters
      --continueOnError Continue after an unexpected case failure
...
`);
  ...
  const outputDirectory = stringOption(values.output, '../../../out/reports/benchmarks/opencascade-brep-surfaces');
