...
import { opencascadeDetectPattern } from '#opencascade.constants.js';

import type { OpenCascadeInstance, TopoDS_Shape } from 'libcascade/init';

const fullWasmUrl = new URL(import.meta.resolve('libcascade/wasm')).href;
const multiWasmUrl = new URL(import.meta.resolve('libcascade/multi/wasm')).href;
...
export type OpenCascadeOptions = {
   ...
   *
   * Defaults to `'full'`. Built-in variants resolve libcascade's exported WASM
   * assets through `new URL(import.meta.resolve(...))`. Custom config keeps the
...
type OpenCascadeContext = {
  ...
  /** Unique MEMFS scratch path per snapshot write, owned by this kernel instance. */
  nextSnapshotPath: (index: number) => string;
};
 ...
 *   package URL expressions let them emit the matching libcascade WASM asset.
...
function registerOcModule(oc: OpenCascadeInstance, runtime: KernelRuntime): void {
  ...
  const exportNames = Object.keys(oc).filter((key) => /^[$_a-z][\w$]*$/i.test(key));
...
function createSnapshotPathAllocator(): (index: number) => string {
  let counter = 0;
  return (index) => {
    counter += 1;
    return `/tmp/tau_opencascade_native_handle_${counter}_${index}.brep`;
  };
}
...
function serializeShapeEntry(
  context: OpenCascadeContext,
  entry: ShapeEntry,
  index: number,
): OpenCascadeSerializedShapeEntry {
  const { oc } = context;
  const filePath = context.nextSnapshotPath(index);
  ...
  try {
    ...
    const rawData = oc.FS.readFile(filePath);
...
/** @public */
export const opencascadeKernel = defineKernel({
  id: 'opencascade',
  extensions: ['ts', 'js'],
  ...
  async initialize(options, runtime) {
    ...
    return {
      oc,
      isParallelMeshing: resolved.variant === 'multi',
      tracingSummary,
      nextSnapshotPath: createSnapshotPathAllocator(),
  ...
  async createGeometry({ entryPath, parameters }, runtime, context) {
    ...
    try {
      ...
      const mainFunction = isRecordObject(module) ? (module['default'] ?? module['main']) : undefined;
