...
describe('@taucad/picovoxel', () => {
  ...
  it('exports the named plugin', async () => {
    ...
    expect(capabilities.kernels.map(({ id }) => id)).toEqual(['picovoxel']);
    ...
    expect(picovoxel({ kernels: { default: { wasm: 'multi' } } }).capabilities.kernels[0].options).toEqual({
      wasm: 'multi',
    });
  });
