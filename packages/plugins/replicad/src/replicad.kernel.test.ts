...
describe('ReplicadWorker', () => {
  ...
  describe('createGeometry', () => {
    ...
    describe('Error handling', () => {
      ...
      it('should return public runtime error details for an undefined variable', async () => {
        const result = await createGeometry({
          files: {
            'main.ts': `
              import {} from 'replicad';
        ...
        expect(result.issues[0]?.stackFrames?.length).toBeGreaterThan(0);
      });
      ...
      it('should report decoded OC exceptions from helper functions', async () => {
...
`;
      ...
      it('should include the OC class name for a fillet exception', async () => {
...
`;
      ...
      it('should include the OC class name for a fillet exception with ocTracing off', async () => {
...
`;
      ...
      it('should preserve public stack categories for a fluent-chain OC exception', async () => {
...
`;
      ...
      it('should classify library frames by export name, not file path', async () => {
        ...
        expect(result.issues[0]).toEqual(
          expect.objectContaining({
            type: 'kernel',
            severity: 'error',
            stackFrames: expect.arrayContaining([expect.objectContaining({ context: 'library' })]),
          }),
        );
  ...
  describe('exportGeometry', () => {
    it('should export to STEP format with actual geometry', async () => {
      ...
      const createResult = await renderGeometry(client, {
        file: geometryFile,
        parameters: {},
      });
    ...
    it.each(['glb', 'gltf'] as const)(
      ...
      async (format) => {
        ...
        await renderGeometry(client, { file: createGeometryFile('coordinate-evidence.ts'), parameters: {} });

        const zUp = await exportLastRender(client, format, {
          coordinateSystem: 'z-up',
          unit: { length: 'millimeter' },
        });
        const yUp = await exportLastRender(client, format, {
  ...
  describe('Stateful kernel runtime', () => {
    it('should deep-merge nested default parameters with user overrides', async () => {
      ...
      const result = await renderGeometry(client, {
        file: geometryFile,
    ...
    it('should re-render with different parameters when replicad is imported transitively (production flow)', async () => {
      ...
      const result1 = await renderGeometry(client, {
        file: geometryFile,
        parameters: { size: 30 },
      });
      ...
      const result2 = await renderGeometry(client, {
        file: geometryFile,
        parameters: { size: 60 },
      });
...
describe('OC API Call Tracing', () => {
  ...
  /** Wait for PerformanceObserver callbacks and the transport telemetry event to settle. */
  async function collectTelemetry(): Promise<void> {
  ...
  it('emits an oc.summary span in summary mode', async () => {
    ...
    const client = createClient(
      { 'box.ts': boxCode },
      {
        workerOptions: { ocTracing: 'summary' },
        onTelemetry: (entries) => telemetryBatches.push(entries),
      },
    );
...
describe('Angular tolerance', () => {
  ...
  it('should produce denser meshes with finer angular tolerance on curved surfaces', async () => {
    const client = createClient({ 'sphere.ts': sphereCode });
    const geometryFile = createGeometryFile('sphere.ts');

    const coarseResult = await renderGeometry(client, {
      file: geometryFile,
      parameters: {},
