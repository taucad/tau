...
describe('@taucad/replicad', () => {
  ...
  it('exports the named plugin', async () => {
    ...
    expect(capabilities.kernels.map(({ id }) => id)).toEqual(['replicad']);
  ...
  it('forwards pthread-free WASM configuration to the selected kernel', () => {
    const selected = replicad({ kernels: { default: { wasm: 'single' } } });

    expect(selected.preset).toBe('default');
    expect(selected.capabilities.kernels.map(({ id }) => id)).toEqual(['replicad']);
    expect(selected.capabilities.kernels[0].options).toEqual({ wasm: 'single' });
  });

  it('defaults OC attribution tracing off while retaining explicit modes', () => {
    expect(replicadOptionsSchema.parse({}).ocTracing).toBe('off');
    expect(replicadOptionsSchema.parse({ ocTracing: 'summary' }).ocTracing).toBe('summary');
    expect(replicadOptionsSchema.parse({ ocTracing: 'per-call' }).ocTracing).toBe('per-call');
  });
