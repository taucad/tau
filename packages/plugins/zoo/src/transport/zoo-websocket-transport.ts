...
import { Topic } from '@taucad/runtime/kernel';
import { binaryToUuid } from '#binary.utils.js';
import { KclError, KclAuthError, KclConnectionError } from '#kcl-errors.js';
...
/**
 * Resolve the platform `WebSocket` constructor.
 *
 * ponytail: no `ws` fallback — every supported host provides a global WebSocket
 * (browsers, and Node from 22.4; this package's floor is `engines.node >= 24`), so the
 * previous dynamic `import('ws')` branch was unreachable and, being undeclared, made
 * tsdown vendor a duplicate `ws` into `dist`. Reinstate it — declaring `ws` — only if
 * the engine floor ever drops below Node 22.4.
 *
 * @returns The global `WebSocket` constructor.
 */
const getWebSocket = (): typeof WebSocket => {
  if (typeof WebSocket === 'undefined') {
    throw new TypeError('WebSocket is unavailable in this environment; @taucad/zoo requires a browser or Node.js 24+.');
  }
