...
import { createMockKernelRuntime, mapZupMillimetersToYupMeters, readCoordinateEvidence } from '@taucad/runtime-testing';
import { resolveRuntimePluginDefinition } from '@taucad/runtime/plugin';

import { writeGlb, writeGltfJson } from '@taucad/geometry-core';
import type { GlbPrimitive } from '@taucad/geometry-core';
  ...
  const runtime = createMockKernelRuntime({
    filesystemOverrides: {
      ...
      readFileResult: async (path) => {
        const content = normalizedFiles[path];
...
async function getParametersWithError(
  ...
  return getParameterResult(files, mainFile);
}
