...
import type { JscadMeshData } from '#kernels/jscad/jscad-to-gltf.js';
import { extractMeshDataFromJscadShape, normalizeJscadShapeForRenderMesh } from '#kernels/jscad/jscad-to-gltf.js';
import { isRenderableJscadPart } from '#kernels/jscad/jscad-parts.js';
...
const { geometries } = resolveJscadModeling(jscadModelingImport);
...
const validateGeom3 = (shape: unknown): ValidationResult => {
  try {
    geometries.geom3.validate(shape as JscadGeom3);
    return { valid: true };
  } catch (error) {
    return {
      valid: false,
...
const polygonCount = (shape: unknown): number | undefined => {
  try {
    return (geometries.geom3.toPolygons(shape as JscadGeom3) as unknown[]).length;
  } catch {
    return undefined;
  }
};
...
const normalizedValidationEvidence = (
  ...
  try {
    const normalizedShape = normalizeJscadShapeForRenderMesh(part.shape, part.index);
    const exportValidation = validateGeom3(normalizedShape);
    return {
      normalizedPolygonCount: polygonCount(normalizedShape),
      exportValidation,
    };
  } catch (error) {
    return {
...
const invalidGeometryIssue = (part: JscadPartDescriptor): KernelIssue | undefined => {
  const nativeValidation = validateGeom3(part.shape);
  const evidence = normalizedValidationEvidence(part);
  ...
  try {
    topology = topologySummaryFromMesh(extractMeshDataFromJscadShape(part.shape, part.index));
  } catch {
  ...
  return {
    ...
    details: {
      ...
      geometry: {
        ...
        polygonCount: polygonCount(part.shape),
 ...
 * @internal
 */
export const collectJscadPartIssues = (parts: readonly JscadPartDescriptor[]): KernelIssue[] => {
  const issues: KernelIssue[] = [];

  for (const part of parts) {
    if (!isRenderableJscadPart(part)) {
      continue;
    }
    const invalidIssue = invalidGeometryIssue(part);
    if (invalidIssue) {
