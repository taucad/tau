...
const conformanceModelSource = `
  import { datum, frame, face } from '@taucad/runtime/kernels/replicad/annotations';
...
const createConformanceWorker = async (): Promise<Awaited<ReturnType<typeof createTestWorker>>> => {
  const worker = await createTestWorker(replicadKernel, { 'main.ts': conformanceModelSource });
  const createResult = await worker.createGeometry({ file: createGeometryFile('main.ts'), parameters: {} });
  assertSuccess(createResult, 'createGeometry for conformance export');
  return worker;
};

const exportStepTextFromWorker = async (worker: Awaited<ReturnType<typeof createTestWorker>>): Promise<string> => {
...
describe('Replicad — AP242 writer conformance', () => {
  it('writes rec-practice supplemental geometry: one aggregated CGR, identity on items, unnamed CGRR with rep_1 = shape rep', async () => {
    const worker = await createConformanceWorker();
    const stepText = await exportStepTextFromWorker(worker);
  ...
  it('produces structurally identical STEP from live, reheated, and deserialized handles', async () => {
    const worker = await createConformanceWorker();
    const internals = worker as unknown as WorkerInternals;

    const liveText = await exportStepTextFromWorker(worker);

    // Deserialized: drop the live handle; export restores from the serialized snapshot.
    expect(internals.currentPublishedRender).toBeDefined();
    internals.currentPublishedRender!.liveNativeHandleSlot = undefined;
    const deserializedText = await exportStepTextFromWorker(worker);

    // Reheat: drop the live handle again plus every serialized slot; export re-runs createGeometry.
    internals.currentPublishedRender!.liveNativeHandleSlot = undefined;
    internals.currentPublishedRender!.serializedNativeHandleSlot = undefined;
    const reheatedText = await exportStepTextFromWorker(worker);
    ...
    expect(extractStepEvidence(deserializedText)).toEqual(liveEvidence);
    expect(extractStepEvidence(reheatedText)).toEqual(liveEvidence);
  });
});
