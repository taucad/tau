...
): Readonly<{ actor: InputActor; emitted: ParameterInputMachineEmitted[]; clock: SimulatedClock }> => {
  const actor = createActor(parameterInputMachine, { input: actorInput, clock });
...
const composeParameterSet = (failFirstApply = false) => {
  let authority: ParameterSetAuthoritySnapshot = {
    identity: revision(),
    entry: fileParameterEntrySchema.parse({
      activeGroup: 'default',
      groups: { default: { values: { width: 25.4 } } },
      identity: revision(),
    }),
  };
  let nextRevision = 1;
  let failApply = failFirstApply;
  const machine = parameterSetMachine.provide({
    actors: {
      resolveParameterSet: fromPromise<ParameterSetAuthoritySnapshot, ParameterSetResolveInput>(async () =>
        structuredClone(authority),
      ),
      planParameterOperation: fromPromise<ParameterSetPlanResult, ParameterSetPlanInput>(async ({ input: plan }) => {
        nextRevision += 1;
        const identity = revision(String(nextRevision));
        const { operation } = plan.request;
        const value =
          operation.kind === 'native-value' ? operation.value : plan.current.entry.groups['default']!.values['width'];
        return {
          status: 'ready',
          proposed: {
            identity,
            entry: fileParameterEntrySchema.parse({
              ...plan.current.entry,
              groups: {
                ...plan.current.entry.groups,
                default: {
                  ...plan.current.entry.groups['default'],
                  values: { ...plan.current.entry.groups['default']!.values, width: value },
                },
              },
              identity,
              lastOperation: {
      ...
      applyParameterOperation: fromPromise<ParameterSetApplyResult, ParameterSetApplyInput>(
        async ({ input: apply }) => {
          if (failApply) {
            failApply = false;
            throw new Error('controlled known-not-applied failure');
          }
          authority = structuredClone(apply.proposed);
          return { status: 'applied', current: structuredClone(authority) };
        },
      ),
      readParameterSet: fromPromise<ParameterSetAuthoritySnapshot, ParameterSetReconcileInput>(async () =>
        structuredClone(authority),
      ),
      flushParameterSet: fromPromise<void, ParameterSetFlushInput>(async () => undefined),
      observeParameterSet: fromCallback<ParameterSetMachineEvent, ParameterSetObserveInput>(() => () => undefined),
    },
  });
  const actor = createActor(machine, { input: { target: binding().target, initialRequestId: 'resolve:input-test' } });
  actor.start();
  return {
    actor,
    submit: async (request: ParameterSetRequest): Promise<ParameterSetOutcome> => {
      actor.send({ type: 'submit', request });
      await waitFor(actor, (snapshot) => snapshot.context.outcome?.requestId === request.requestId);
      return structuredClone(actor.getSnapshot().context.outcome!);
    },
  };
};
...
describe('parameterInputMachine', () => {
  ...
  it('should expose malformed runtime input as a serializable failed state', () => {
    ...
    expect(actor.getSnapshot().context.diagnostic).toMatchObject({ code: 'METADATA_CONFLICT' });
    expect(() => JSON.stringify(actor.getSnapshot().context)).not.toThrow();
    expect(emitted).toEqual([]);
    actor.stop();
  });
  ...
  it.each([
    { text: '', state: 'incomplete', result: 'incomplete', code: 'UNIT_INVALID' },
    { text: '-', state: 'incomplete', result: 'incomplete', code: 'UNIT_INVALID' },
    { text: '1/', state: 'invalid', result: 'invalid', code: 'UNIT_INVALID' },
    { text: '1e999', state: 'invalid', result: 'invalid', code: 'NUMERIC_OVERFLOW' },
    { text: '1 splonk', state: 'invalid', result: 'invalid', code: 'UNIT_INVALID' },
    ...
    expect(actor.getSnapshot().context.draft).toMatchObject({ raw: text, resultStatus: result });
    expect(actor.getSnapshot().context.draft?.diagnostic).toMatchObject({ code });
  ...
  it('should keep unknown numeric semantics distinct from explicit dimensionless semantics', () => {
    ...
    const suffixedUnknown = startActor(
      input({ binding: unknownBinding(), acknowledgedValue: 2, display: { locale: 'en' } }),
    );
    ...
    expect(dimensionless.emitted[0]?.request.operation).toMatchObject({ kind: 'unit-value', inputUnit: '1' });
  ...
  it('should emit one captured unit-value intent on Enter and acknowledge only its committed outcome', () => {
    ...
    actor.send({ type: 'settleSubmission', generation: 999, outcome: committed(emitted[0]!) });
    expect(actor.getSnapshot().context.acknowledged.value).toBe(25.4);
    actor.send({
  ...
  it('should emit nothing for focus, blur, Escape, no-op Enter, display change, and clean refresh', () => {
    ...
    actor.send({ type: 'changeDisplay', display: { unit: 'cm', locale: 'de-DE' } });
    actor.send({ type: 'refreshAuthority', binding: binding(), value: 25.4, revision: revision('2') });
    actor.send({ type: 'focus' });
    actor.send({ type: 'pressEnter' });
    expect(interaction(actor, 'viewing')).toBe(true);
  ...
  it('should preserve captured unit and locale across display changes while editing', () => {
    ...
    expect(emitted).toEqual([]);
    actor.send({ type: 'pressEnter' });
    expect(emitted[0]).toMatchObject({ locale: 'en-NZ', request: { operation: { inputUnit: '[in_i]', value: '2' } } });
    actor.stop();
  });
  ...
  it('should refresh clean data and retain a dirty draft in rebind conflict', () => {
    const { actor, emitted } = startActor();
    actor.send({ type: 'focus' });
    actor.send({ type: 'refreshAuthority', binding: binding(), value: 50.8, revision: revision('2') });
    expect(editing(actor, 'complete-valid')).toBe(true);
    expect(actor.getSnapshot().context.acknowledged.value).toBe(50.8);
    actor.send({ type: 'changeRaw', text: '3 in' });
    actor.send({ type: 'refreshAuthority', binding: binding(), value: 63.5, revision: revision('3') });
  ...
  it.each([
    ...
    { name: 'structurally changed', next: binding({ pointer: '/items/0/width' }), reason: 'binding-changed' },
    ...
    expect(interaction(actor, 'conflicted')).toBe(true);
    expect(actor.getSnapshot().context.draft?.conflict).toEqual({ reason, rebindable: false });
    actor.send({ type: 'rebind' });
    actor.send({ type: 'pressEnter' });
    expect(interaction(actor, 'conflicted')).toBe(true);
    expect(emitted).toEqual([]);
  ...
  it('should bound continual mode to one active plus the latest pending generation', () => {
    ...
    actor.send({ type: 'settleSubmission', generation: first.request.draftGeneration, outcome: committed(first, '2') });
    const latest = emitted[1]!;
    expect(latest.request).toMatchObject({ pressure: 'final', operation: { kind: 'native-value', value: 50 } });
  ...
  it('should compose continual settlement through the real parameter-set owner with the committed revision', async () => {
    const parameterSet = composeParameterSet();
    await waitFor(parameterSet.actor, (snapshot) => snapshot.matches({ operational: 'ready' }));
    const inputActor = startActor(input({ pressure: 'continual', display: { unit: 'mm', locale: 'en' } }));
  ...
  it('should preserve unique request identity after a real known-not-applied settlement and discard', async () => {
    const parameterSet = composeParameterSet(true);
    await waitFor(parameterSet.actor, (snapshot) => snapshot.matches({ operational: 'ready' }));
    const inputActor = startActor(input({ display: { unit: 'mm', locale: 'en', increment: 1 } }));
    ...
    expect(secondOutcome).toMatchObject({ status: 'committed', requestId: second.request.requestId });
    inputActor.actor.stop();
    parameterSet.actor.stop();
  });
  ...
  it('should retain an active settlement barrier through refresh, rebind, discard, and focus', () => {
    ...
    expect(emitted).toHaveLength(1);
    expect(actor.getSnapshot().context.submission?.active.request.requestId).toBe(active.request.requestId);
    actor.send({ type: 'refreshAuthority', binding: binding(), value: 50.8, revision: revision('9') });
    actor.send({ type: 'rebind' });
  ...
  it('should preserve refreshed authority when an obsolete binding settles late', () => {
    ...
    actor.send({ type: 'refreshAuthority', binding: renamed, value: 90, revision: revision('9') });
    actor.send({
      type: 'settleSubmission',
      generation: active.request.draftGeneration,
  ...
  it('should retain an indeterminate barrier across Escape until a matching reconciliation settles', () => {
    ...
    actor.send({
      type: 'settleSubmission',
      generation: active.request.draftGeneration,
      outcome: { status: 'indeterminate', requestId: active.request.requestId, code: 'ACK_LOST', message: 'lost' },
    });
    actor.send({ type: 'pressEscape' });
    actor.send({ type: 'step', direction: 1 });
    expect(emitted).toHaveLength(1);
  ...
  it('should convert fractional pointer and keyboard numbers without locale reinterpretation', () => {
    ...
    expect(pointer.emitted[0]?.request.operation).toMatchObject({ kind: 'native-value', value: 1.5 });
    expect(pointer.actor.getSnapshot().context.draft?.raw).toBe('1,5');
    pointer.actor.stop();

    const keyboard = startActor(
      input({ acknowledgedValue: 1, display: { unit: 'mm', locale: 'de-DE', increment: 0.5 } }),
    );
    keyboard.actor.send({ type: 'step', direction: 1 });
    expect(keyboard.emitted[0]?.request.operation).toMatchObject({ kind: 'native-value', value: 1.5 });
  ...
  it.each(['50 %', '3 mm', '3 1'])('should reject explicit suffix %s when numeric semantics are unknown', (text) => {
    ...
    expect(actor.getSnapshot().context.draft?.diagnostic).toMatchObject({ code: 'SEMANTICS_UNRESOLVED' });
    actor.send({ type: 'pressEnter' });
    expect(emitted).toEqual([]);
    actor.stop();
  });
  ...
  it('should keep a removed binding unavailable through discard and restore it only on valid refresh', () => {
    const { actor, emitted } = startActor();
    actor.send({ type: 'refreshAuthority', value: 25.4, revision: revision('2') });
    actor.send({ type: 'discard' });
    actor.send({ type: 'focus' });
    actor.send({ type: 'changeRaw', text: '2 in' });
    actor.send({ type: 'pressEnter' });
  ...
  it.each([
    { type: 'changeDisplay', display: { unit: 'mm', locale: 'en', increment: () => 1 } },
  ...
  it.each([
    { status: 'rejected', code: 'STALE_MANIFEST', expected: 'conflicted', retry: false },
    { status: 'known-not-applied-failure', code: 'WRITE_FAILED', expected: 'failed', retry: true },
    { status: 'indeterminate', code: 'ACK_LOST', expected: 'failed', retry: false },
    ...
    actor.send({
      ...
      outcome: { status, requestId: intent.request.requestId, code, message: code },
    });
  ...
  it('should retain acknowledgement when submission is cancelled before apply', () => {
    ...
    actor.send({
      type: 'settleSubmission',
      generation: intent.request.draftGeneration,
      outcome: { status: 'cancelled-before-apply', requestId: intent.request.requestId },
    });
    expect(interaction(actor, 'viewing')).toBe(true);
    expect(actor.getSnapshot().context.acknowledged.value).toBe(25.4);
    actor.stop();
  ...
  it('should apply constrained arrow intent with captured modifiers', () => {
    const { actor, emitted } = startActor(
      input({
        binding: binding({ constraints: { minimum: 0, maximum: 10, multipleOf: 0.5 } }),
        acknowledgedValue: 9,
    ...
    expect(interaction(actor, 'submitting')).toBe(true);
    expect(emitted[0]?.request.operation).toMatchObject({ kind: 'native-value', value: 10 });
    actor.stop();
  });

  it('should keep validation multipleOf separate from the display increment', () => {
    const { actor, emitted } = startActor(
      input({
        binding: binding({ constraints: { multipleOf: 10 } }),
        acknowledgedValue: 25.4,
        display: { unit: '[in_i]', locale: 'en', increment: 0.5 },
      }),
    );
    actor.send({ type: 'step', direction: 1 });
    expect(emitted[0]?.request.operation).toMatchObject({ kind: 'native-value', value: 38.1 });
    actor.stop();
  });
  ...
  it('should release clean detach and retain dirty or in-flight state until discard or close', () => {
    ...
    expect(clean.actor.getSnapshot().matches({ active: { attachment: 'detached', interaction: 'viewing' } })).toBe(
      true,
    );
    ...
    expect(discarded.actor.getSnapshot().matches({ active: { attachment: 'detached', interaction: 'viewing' } })).toBe(
      true,
    );
