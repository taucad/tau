...
const semanticKeywords = new Set(['x-ogc-unit', 'x-tau-quantity-kind', 'x-tau-reference', 'x-tau-space', 'x-tau-unit']);
...
const createProjection = (
  ...
  const visit = ({ value, mode, instancePointer, emit }: VisitInput): unknown => {
    ...
    for (const [key, child] of Object.entries(value)) {
      ...
      if (schemaArrayKeywords.has(key) && Array.isArray(child)) {
        projected[key] = child.map((schema) => visit({ value: schema, mode: 'unsupported', instancePointer, emit }));
        continue;
      }
      if (schemaKeywords.has(key)) {
        projected[key] = Array.isArray(child)
          ? child.map((schema) => visit({ value: schema, mode: 'unsupported', instancePointer, emit }))
  ...
  const schema = visit({ value: root, mode: 'exact', instancePointer: '', emit: true });
