...
const declaration = (): ParameterDeclaration => ({
  schema: {
    ...
    properties: {
      ...
      optionalLength: { type: [{ $ref: '#/definitions/nullableLength' }, 'null'] },
...
const compile = async (value = declaration(), resolution = {}) =>
  compileParameterManifest({
    declaration: value,
...
describe('native parameter manifest', () => {
  it('admits the public draft token, derives stable bindings, and preserves native values', async () => {
    ...
    expect(manifest.defaults).toEqual({ length: 2.5, copies: 2, precise: '0.1' });
    expect(manifest.dialect).toEqual({ core: '-04', units: '-03', activation: 'JSONSchemaUnits' });
    ...
    expect(manifest.bindings['/optionalLength']).toMatchObject({ nullable: true, unit: 'mm' });
    expect(manifest.bindings['/precise']).toMatchObject({ representation: 'decimal', unit: 'm' });
  ...
  it('includes resolution settings in manifest identity and revision', async () => {
    const first = await compile(declaration(), { mode: 'default', inferenceLanguage: 'en-NZ' });
    const second = await compile(declaration(), { mode: 'declared-only', inferenceLanguage: 'en-NZ' });
    expect(first.identity.resolution).toEqual({ mode: 'default', inferenceLanguage: 'en-NZ' });
  ...
  it.each([
    ...
    [
      ...
      { properties: { length: { type: 'double', ucumUnit: 'definitely-not-a-unit' } } },
      'INVALID_ANNOTATION',
    ],
    [
    ...
    [
      ...
      { properties: { length: { type: 'double', ucumUnit: 'mm', symbols: { 'lang:not_a_tag': 'x' } } } },
      'INVALID_ANNOTATION',
    ],
    [
      'unknown required semantics',
      { properties: { length: { type: 'double', ucumUnit: 'mm', unimplementedRequiredSemantics: true } } },
      'INVALID_SCHEMA',
    ],
  ...
  it('rejects direct nullable unit annotations and accepts the qualified reference form', () => {
    ...
    direct.schema['properties'] = { value: { type: ['double', 'null'], ucumUnit: 'mm' } };
  ...
  it('derives a valid nullable Draft-7 OGC projection with fixed integer bounds', () => {
    const projectable = structuredClone(declaration().schema) as { properties: Record<string, unknown> };
    ...
    expect(projected.schema).toMatchObject({
      ...
      properties: {
        length: { type: 'number', 'x-ogc-unit': 'mm', 'x-ogc-unitLang': 'UCUM' },
        copies: {
          type: 'integer',
  ...
  it('bounds generic integers to the safe executable range', () => {
    const schema = {
      ...
      name: 'SafeInteger',
      type: 'object',
      properties: { count: { type: 'integer', ucumUnit: '1', minimum: -10_000_000_000_000_000 } },
    };
  ...
  it('resolves supplied resources offline and diagnoses cycles and missing resources', async () => {
    ...
    cyclic.schema['definitions'] = { loop: { type: { $ref: '#/definitions/loop' } } };
    cyclic.schema['properties'] = { loop: { type: { $ref: '#/definitions/loop' } } };
  ...
  it('canonically re-admits tables and rejects forged authority with stale revisions', async () => {
    ...
    const changedUnit = {
      ...manifest,
      bindings: { ...manifest.bindings, '/length': { ...manifest.bindings['/length']!, unit: 'cm' } },
    };
    await expect(admitParameterManifest(changedUnit)).rejects.toThrow(ParameterAdmissionError);

    const changedSource = { ...manifest, source: { ...manifest.source, id: 'forged-kernel' } };
    ...
    const changedIdentity = {
      ...
      bindings: {
        ...
        '/length': {
          ...
          parameter: { ...manifest.bindings['/length']!.parameter, value: 'forged:parameter' },
    ...
    const invalidDefaults = { ...manifest, defaults: { length: -5, copies: 1000, precise: 4 } };
    await expect(admitParameterManifest(invalidDefaults)).rejects.toThrow(ParameterAdmissionError);
    expect(() => admitParameterDeclaration({ ...declaration(), defaults: invalidDefaults.defaults })).toThrow(
      ParameterAdmissionError,
    );
    ...
    const changedProjection = structuredClone(projectedManifest) as unknown as {
      legacyProjection: { schema: { properties: { length: Record<string, unknown> } } };
    };
  ...
  it('requires exact producer resource structure while allowing attributed binding enrichment', async () => {
    ...
    await expect(
      ...
      diagnostics: [expect.objectContaining({ code: 'METADATA_CONFLICT', schemaPointer: '/resources' })],
    });
  });
  ...
  it.each([
    [
      ...
      { origin: 'inferred', producer: 'rule', sourceRevision: digest, rule: 'length', evidence: 'length' },
    ],
    [
    ...
    ['empty project evidence', { origin: 'project', producer: 'project.json', sourceRevision: digest, evidence: '' }],
  ...
  it.each([
    ...
    expect(Object.values(admitted.provenance)).toContainEqual({ field: 'unit', ...provenance });
  });
  ...
  it.each([
    ...
    expect(admitted.bindings['/length']).toMatchObject({ unit: 'mm', space: 'point' });
    expect(admitted.bindings['/width']).toMatchObject({ unit: 'cm', space: 'point' });
  ...
  it('bounds hostile schemas and rejects malformed post-boundary tables', async () => {
    ...
    expect(() =>
      admitParameterDeclaration({ ...oversized, schema: { ...oversized.schema, properties: { nested } } }),
    ).toThrow('PARAMETER_MANIFEST_LIMIT');

    const manifest = await compile();
    await expect(admitParameterManifest({ ...manifest, bindings: { '/length': { unit: 'mm' } } })).rejects.toThrow(
      ParameterAdmissionError,
    );
    await expect(
      admitParameterManifest({ ...manifest, identity: { ...manifest.identity, resolution: { mode: 'invented' } } }),
    ).rejects.toThrow(ParameterAdmissionError);
  });
});
