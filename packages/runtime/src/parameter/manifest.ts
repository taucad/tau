...
export type ParameterManifest = Readonly<{
  version: 1;
  profile: typeof unitProfile;
  dialect: Readonly<{ core: '-04'; units: '-03'; activation: 'JSONSchemaUnits' }>;
  schema: JsonStructureSchema;
  resources: Readonly<Record<string, JsonStructureSchema>>;
  defaults: Readonly<Record<string, unknown>>;
...
const qualifyBindingClaim = (
  location: Readonly<{ resource: string; schemaPointer: string; instancePointer: string }>,
...
): ParameterDiagnostic => ({ code, message, severity: 'error', resource, schemaPointer, ...details });
...
const validateSchema = (
  ...
  const result = new SchemaValidator({ extended: true, allowImport: true, externalSchemas: external }).validate(
    copySchemaForSdk(schema),
  );
  if (!result.isValid) {
...
const validateDefaults = (declaration: ParameterDeclaration): void => {
  ...
  const schema = copySchemaForSdk(
    inlineExternalReferences(declaration.schema, rootResource) as JsonStructureSchema,
...
const parseReference = (
  ...
  return hash === -1
    ...
    : { resource: reference.slice(0, hash), pointer: reference.slice(hash + 1) };
};
...
const resolveBindingUnit = (
  ...
  context: Readonly<{ types: readonly string[]; resource: string; pointer: string }>,
): string | undefined => {
...
const deriveManifestTables = (
  ...
  while (pending.length > 0) {
    ...
    if (typeof reference === 'string') {
      ...
      pending.push({ ...current, node: targetNode, resource: target.resource, schemaPointer: target.pointer });
      continue;
    }
    ...
    if (unit !== undefined) {
      const parameter: ParameterIdentity = declared.parameterId
        ...
        : { value: `${source.revision}:${current.instancePointer}`, stability: 'revision-scoped' };
      const constraints = Object.fromEntries(constraintKeys.flatMap((key) => (key in node ? [[key, node[key]]] : [])));
      bindings[current.instancePointer] = {
        parameter,
      ...
      if (types.includes('decimal')) {
        diagnostics.push({
          ...diagnostic(
            ...
            current.resource,
            current.schemaPointer,
            { instancePointer: current.instancePointer, parameterId: parameter.value },
          ),
          severity: 'warning',
        });
      }
...
export const projectParameterSchemaToDraft7 = (schema: JsonStructureSchema): ParameterLegacyProjection => {
  ...
  return deepFreeze(
    usable
      ? { status: 'usable', dialect: 'draft-07', schema: projectedSchema, diagnostics }
...
const cloneDeclaration = (value: unknown): ParameterDeclaration => {
  ...
  if (bindingDeclarations !== undefined) {
    ...
    for (const pointer of Object.keys(bindingsRecord)) {
      ...
      assertOnlyKeys(
        bindingRecord,
        new Set(['parameterId', 'unit', 'quantityKind', 'space', 'reference', 'provenance']),
        `/bindings/${escapePointer(pointer)}`,
      );
...
export const compileParameterManifest = async (input: CompileParameterManifestInput): Promise<ParameterManifest> => {
  ...
  const withoutRevision = {
    ...
    scope,
    source,
    identity: { dependency: input.dependency, middleware: input.middleware, resolution },
  } as const;
...
const inspectParameterManifest = (value: unknown): ParameterManifest => {
  ...
  if (
    canonicalizeCacheValue({ value: asCacheValue(candidate['legacyProjection']) }) !==
  ...
  for (const [pointer, binding] of Object.entries(bindings)) {
    ...
    if (
      ...
      (bindingRecord['representation'] !== expected.representation ||
        ...
        canonicalizeCacheValue({ value: asCacheValue(bindingRecord['constraints']) }) !==
...
/** Synchronous structural guard for the wire codec; it does not grant semantic admission. @internal */
