...
import { calculateFovLightingCompensation } from '#utils/math.utils.js';
 ...
 *
 * @param options - Headlamp transform options.
 * @param options.cameraPosition - The camera's world position.
 * @param options.cameraMatrixWorld - The camera's world matrix (used for basis vectors).
 * @param options.sceneRadius - The bounding sphere radius of the scene.
 * @param options.config - Offset multipliers for headlamp placement.
 * @returns The world-space position and target position for the headlamp.
 */
export function computeHeadlampTransform(options: {
  cameraPosition: THREE.Vector3;
  cameraMatrixWorld: THREE.Matrix4;
  sceneRadius: number;
  config: HeadlampConfig;
}): { position: THREE.Vector3; targetPosition: THREE.Vector3 } {
  const { cameraPosition, cameraMatrixWorld, sceneRadius, config } = options;
