...
discover_scopes() {
  local scopes=("root")
  for dir in "$REPO_ROOT"/apps/*/project.json "$REPO_ROOT"/apps/libs/*/project.json "$REPO_ROOT"/packages/*/project.json "$REPO_ROOT"/packages/plugins/*/project.json "$REPO_ROOT"/packages/core/*/project.json "$REPO_ROOT"/libs/*/project.json; do
    [ -f "$dir" ] || continue
    local name
...
detect_touched_scopes() {
  ...
  while IFS= read -r file; do
    ...
    for scope in "${all_scopes[@]}"; do
      [ "$scope" = "root" ] && continue
      if [[ "$file" == apps/"$scope"/* ]] || [[ "$file" == apps/libs/"$scope"/* ]] || [[ "$file" == packages/"$scope"/* ]] || [[ "$file" == packages/plugins/"$scope"/* ]] || [[ "$file" == packages/core/"$scope"/* ]] || [[ "$file" == libs/"$scope"/* ]]; then
        touched+=("$scope")
        matched=true
        break
      fi
...
build_scope_map() {
  ...
  for scope in "${scopes[@]}"; do
    ...
    elif [ -d "$REPO_ROOT/packages/plugins/$scope" ]; then
      map+="  - $scope: packages/plugins/$scope/"$'\n'
    elif [ -d "$REPO_ROOT/packages/core/$scope" ]; then
      map+="  - $scope: packages/core/$scope/"$'\n'
